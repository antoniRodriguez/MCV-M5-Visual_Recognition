...
for to_evaluate in models:
    ...
    # Train
    lr = 0.0025
    # region_proposals = 1000
    # EXPERIMENT_NAME = f"{to_evaluate[:-5]}_trained_RegionProposals_"+str(region_proposals)
    ...
    # This is the total number of RPN proposals produced (for both FPN and non-FPN cases)
    #cfg.MODEL.RPN_POST_NMS_TOP_N = region_proposals #default 2000
    # cfg.MODEL.RPN.PRE_NMS_TOPK_TRAIN = region_proposals

    # Aspect ratios define the shapes of anchors. 
    # The five elements of ANCHOR_GENERATOR.SIZES list correspond to five levels of feature 
    # maps (P2 to P6). For example P2 (stride=4) has one anchor whose size is 32.
    ...
    # values = [i*0.1 for i in range(1,10)]
    # anchors = {'8-128':[[8],[16],[32],[64],[128]], '64-768':[[64],[128],[256],[512],[768]], '16-64-128-512-1024':[[16], [64], [128],[512],[1024]]}
    # ratios = {'02-05-07':[[0.2, 0.5, 0.7]], '10-20-30':[[1.0,2.0,3.0]], '02-10-30':[[0.2,1.0,3.0]]}
    region_proposals = [500, 1000, 1500, 2000]

    for value in region_proposals:

        hyper = 'PROPOSALS_'+str(value)
        cfg.MODEL.RPN.PRE_NMS_TOPK_TRAIN = value
        EXPERIMENT_NAME = f"{to_evaluate[:-5]}_trained_{hyper}"
        OUTPUT_DIR = f"/home/group00/working/week4/model_evaluation_hyper/{EXPERIMENT_NAME}"
