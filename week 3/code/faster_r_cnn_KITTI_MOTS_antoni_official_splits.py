...
lr = 0.00025
EXPERIMENT_NAME = '3000_iter_1024B_'+'000025'+'LR_std_config_with_loss'
...
test_paths = [base_path+str(i).zfill(4)+'.txt' for i in [16,18]]
...
for d in [train_paths]:
    ...
    MetadataCatalog.get('train_kitti-mots').set(thing_classes=['Car',
                        'Pedestrian'])

#val dataset
for d in [val_paths]:
    DatasetCatalog.register('val_kitti-mots', lambda d=d:get_kitti_dataset_val(d))
    MetadataCatalog.get('val_kitti-mots').set(thing_classes=['Car',
                        'Pedestrian'])

#test dataset
for d in [test_paths]:
    DatasetCatalog.register('test_kitti-mots', lambda d=d:get_kitti_dataset_val(d))
    MetadataCatalog.get('test_kitti-mots').set(thing_classes=['Car',
                        'Pedestrian'])
...
OUTPUT_DIR = "/home/group00/working/week3/models/"+EXPERIMENT_NAME
...
#EVALUATION
evaluator = COCOEvaluator("test_kitti-mots", cfg, False, output_dir="../week3/test_set/")
val_loader = build_detection_test_loader(cfg, "test_kitti-mots")
